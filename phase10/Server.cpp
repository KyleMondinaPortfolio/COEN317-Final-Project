...
void Server::handle_client_interests(int connfd, int server_buffer_size, Interests *interests){
	int client_socket = connfd;
	int valread;
	char server_buffer[server_buffer_size] = {0};
...
void Server::handle_client_node_interests(int connfd, int server_buffer_size, NodeList *nodes, Interests *interests){
	int client_socket = connfd;
	int valread;
	char server_buffer[server_buffer_size] = {0};
...
void Server::start_interests(Interests *interests)
{
	int connfd;
	struct sockaddr_in clientAddr;
	socklen_t alen;
	...
	while (1){
		...
		//dispatch a thread to process the request
		std::thread t(handle_client_interests,connfd,server_buffer_size,interests);
		t.detach();
	}	
}
...
void Server::start_node(NodeList *nodes, Interests *interests)
{
	int connfd;
	struct sockaddr_in clientAddr;
	socklen_t alen;
	while (1){
		...
		std::thread t(handle_client_node_interests,connfd,server_buffer_size,nodes,interests);
		t.detach();
