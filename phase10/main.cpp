...
void user_interface(Interests *interests,UDPNodeList *friends,Client *server, bool *server_online, TimeStamp *ts, std::mutex *mtx, MessageIDBuffer *mbuffer){
	...
	while(1){
		...
		int target_id;
		int message_id = rand();
		std::string update_interests;
		std::string hobby;
		std::string haddress;

		std::cout << "----User Interface-----" << std::endl; 

		std::cout << "Do You Want To Update Your Interests?" << std::endl; 
		std::getline(std::cin,update_interests);

		if (update_interests == "ADD"){
			std::cout << "What Hobby?" << std::endl; 
			std::getline(std::cin,hobby);
			std::cout << "What Address?" << std::endl; 
			std::getline(std::cin,haddress);
			interests->add(guid,hobby,haddress);
		}else if (update_interests == "REMOVE"){
			std::cout << "What Hobby?" << std::endl; 
			std::getline(std::cin,hobby);
			std::cout << "What Address?" << std::endl; 
			std::getline(std::cin,haddress);
			interests->remove(guid,hobby,haddress);
		}
...
int main(){
	...
	if (server_type == 0){
		...
		//Main Listener
		std::thread t2(&Server::start_interests, &self_main, &interests);
		...
		TimeStamp ts;
		std::mutex tsmtx;
		std::thread t3(user_interface,&interests,&friends,&central_server,&server_online,&ts,&tsmtx,&mbuffer);
		...
		//for port 8000
		//Main Server
		std::thread t1(&Server::start_node,&self_main,&nodes,&interests);
