...
int main(){
	...
	if (server_type == 0){
		...
		Server self(guid,8000,100,1000);
		...
		//User Interface
		std::thread t2(user_interface, &central_server, &server_online);

		//Background Listening Process
		std::thread t3(&Server::start_server_online, &self, &server_online);
		...
		//Background Listening Process
		std::thread t1(&Server::start_node,&s,&nodes);
